...
(function() {
  ...
  const sendIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>';
  ...
  container.innerHTML = \`
      ...
      <div class="wa-header">
        <div style="display: flex; align-items: center; gap: 12px;">
          <div style="position: relative;">
            <img src="\${config.profilePicture}" alt="Profile" style="width: 48px; height: 48px; border-radius: 50%; object-fit: cover; border: 2px solid rgba(255,255,255,0.2);">
            <div style="position: absolute; bottom: 0; right: 0; width: 12px; height: 12px; border-radius: 50%; border: 2px solid #fff; background-color: \${config.colors.userOnlineStatus};"></div>
          </div>
          <div>
            <div style="font-weight: 600; font-size: 16px;">\${config.name}</div>
            <div style="font-size: 13px; opacity: 0.9;">\${config.caption}</div>
          </div>
        </div>
      ...
      <div class="wa-input-area">
        \${config.startChatMethod === 'message' ? \`
          <div class="wa-input-container">
            <input type="text" id="wa-input" class="wa-input" placeholder="Type a message...">
    ...
    <button class="wa-widget-bubble" id="wa-bubble">
      <div style="position: relative;">
        \${waIcon}
        <div style="position: absolute; top: -4px; right: -4px; width: 12px; height: 12px; border-radius: 50%; border: 2px solid #fff; background-color: \${config.colors.bubbleNotificationBadge};"></div>
      </div>
      \${config.bubbleText ? \`<span style="font-weight: 500; font-size: 16px;">\${config.bubbleText}</span>\` : ''}
    </button>
    \` : ''}
  \`;
  ...
  if (startBtn) {
    startBtn.addEventListener('click', () => openWhatsApp(''));
  }
})();
</script>`;
  };
